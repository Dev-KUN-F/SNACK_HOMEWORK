...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
     ...
     ArrayList<BookRentDto> dtos = dao.getBookRentList( search );
...
<head>
<meta charset="UTF-8">
<title>대여 목록</title> <style type="text/css">
...
<script type="text/javascript">
	...
	function goBook(){
		book.method="post";
		book.action="../book/book_list.jsp"
		book.submit();
	}
	
	function return_book(rcode,bcode){
		 let con = confirm("반납 처리 하시겠습니까?");

			        if(con){
			            // hidden 필드에 rcode와 bcode 값 설정
			            document.book.t_rcode.value = rcode;
			            document.book.t_bcode.value = bcode;

			            book.method="post";
			            book.action="db_return.jsp"; // action은 db_return.jsp로 직접 지정
			            book.submit();
			        }else{
			            alert("반납취소!");
			        }
...
<form name="book">
	<input type="hidden" name="t_rcode" value=""/>
    <input type="hidden" name="t_bcode" value=""/>
...
<table border ="1" width="100%">
	...
	<%for(BookRentDto dto : dtos){ %>
	
	<tr>
		<td align="center"><%= dto.getR_code()%></td>
		<td align="center"> <%= dto.getB_name()%></td>
		<td align="center"><%= dto.getM_name()%></td>
		<td align="center"><%= dto.getRent_date() %></td>
		<td align="center"><% if(dto.getReturn_date() == null){
			out.print("대여중!");
		}else{
			out.print("반납완료!");
		} %></td>
		<td align ="center">
		 <% if(dto.getReturn_date() == null) {%>
    		
       		<input type="button" onclick="return_book('<%=dto.getR_code() %>', '<%=dto.getB_code() %>');" value="반납!"/>
       		
    	<%}else{%>
    		<span><%= dto.getReturn_date() %></span>
    	<%} %>
