...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
     ...
     <%
		...
		BookDto dto = dao.getBookInfo(bcode);
...
<head>
<meta charset="UTF-8">
<title>도서 수정</title> <style type="text/css">
...
<script type="text/javascript">
	function goUpdate(){
		...
		book.action = "db_book_update.jsp";
		book.submit();
	}
...
<table border = "1" width="400">
		...
		<tr>
			...
			<td>
				<%=dto.getB_code() %>
				<input  type="hidden" name = "t_bcode" value = "<%=dto.getB_code()%>"/>
			</td>
		</tr>
		...
		<tr>
			<th>도서명</th>
			<td><input type="text" name ="t_bname" size = "10" value="<%=dto.getB_name()%>"/></td>
		</tr>
		
		<tr>
			<th>출판사</th>
			<td>
				<input type="text" name = "t_pub" size = "10" value ="<%=dto.getPublisher() %>" />
			</td>
		</tr>
		
		<tr>
			<th>저자</th>
			<td><input type="text"   name ="t_writer"value="<%=dto.getWriter()%>"/></td>
		</tr>
		
		<tr>
			<th>등록 일자</th>
			<td><input type="date"   name ="t_regDate"  value="<%=dto.getReg_date()%>"/></td>
		</tr>
